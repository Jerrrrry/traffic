...
def get_ip():
    try:
        ...
        token=get_token()
        url = 'https://api.getproxylist.com/proxy?country[]=US&lastTested=600&maxConnectTime=1&protocol[]=socks4&protocol[]=socks5h&protocol[]=socks4a&apiKey='+token
...
def get_proxy():
    try:
        # 这里填写大象代理api地址，num参数必须为1，每次只请求一个IP地址
        token=get_token()
        url = 'https://api.getproxylist.com/proxy?country[]=US&protocol[]=socks4&protocol[]=socks5&protocol[]=socks5h&protocol[]=socks4a&apiKey='+token
        response = requests.get(url)
        response.close()
